...
public class Admin extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		       ...
		       //브라우저에게 html이 utf-8형식이라고 전달
				...
				PrintWriter out = response.getWriter();  //out을 변수로 설정 
				//서블릿에서 만드는 html파일의 인코딩이 utf-8이다
				response.setCharacterEncoding("utf-8");
				request.setCharacterEncoding("utf-8");
				
				String id = request.getParameter("userid"); //클라이언트의 요청 정보를 request에 담는다!
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		doGet(request, response); //dopost에서 비밀번호 패스워드를 받아서 doget으로 받는다. 그래서 보이지가 않음
	}
